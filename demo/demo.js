...
$(function () {
  ...
  // fillMode values: solid | outline | flat | link | clear
  var allFillModes = ['solid', 'outline', 'flat', 'link', 'clear'];

  // themeColor values: base | primary | secondary | tertiary | info | success | warning | error | dark | light | inverse
  var allThemeColors = ['base', 'primary', 'secondary', 'tertiary', 'info', 'success', 'warning', 'error', 'dark', 'light', 'inverse'];

  // 1. Fill modes (all 5, primary color)
  allFillModes.forEach(function (fm) {
    var $b = $('<button>').text(cap(fm));
    $('#btn-fillmodes-row').append($b);
    $b.kendoButton({ fillMode: fm, themeColor: 'primary' });
  });

  // 2–6. All theme colors × each fill mode
  var fillColorMap = {
    'btn-solid-colors-row':   'solid',
    'btn-outline-colors-row': 'outline',
    'btn-flat-colors-row':    'flat',
    'btn-link-colors-row':    'link',
    'btn-clear-colors-row':   'clear',
  };

  Object.keys(fillColorMap).forEach(function (rowId) {
    var fm = fillColorMap[rowId];
    allThemeColors.forEach(function (color) {
      var $b = $('<button>').text(cap(color));
      $('#' + rowId).append($b);
      $b.kendoButton({ fillMode: fm, themeColor: color });
    });
  });
  ...
  [
    ...
    var $b = $('<button>').text(cfg.label);
    $('#btn-sizes-row').append($b);
    $b.kendoButton({ size: cfg.size, themeColor: 'primary', fillMode: 'solid' });
  });

  // 8. Border radius (rounded): sm | md | lg | full
  [
    { rounded: 'sm',   label: 'Rounded sm'   },
    { rounded: 'md',   label: 'Rounded md'   },
    { rounded: 'lg',   label: 'Rounded lg'   },
    { rounded: 'full', label: 'Rounded full' },
  ].forEach(function (cfg) {
    var $b = $('<button>').text(cfg.label);
    $('#btn-rounded-row').append($b);
    $b.kendoButton({ rounded: cfg.rounded, themeColor: 'primary', fillMode: 'solid' });
  });

  // 9. Íconos + texto
  [
    { icon: 'plus',     text: 'Agregar',    color: 'primary',   fill: 'solid'   },
    { icon: 'pencil',   text: 'Editar',     color: 'secondary', fill: 'solid'   },
    { icon: 'trash',    text: 'Eliminar',   color: 'error',     fill: 'solid'   },
    { icon: 'check',    text: 'Confirmar',  color: 'success',   fill: 'solid'   },
    { icon: 'download', text: 'Exportar',   color: 'info',      fill: 'outline' },
    { icon: 'upload',   text: 'Importar',   color: 'primary',   fill: 'outline' },
    { icon: 'refresh',  text: 'Actualizar', color: 'base',      fill: 'flat'    },
    { icon: 'eye',      text: 'Ver',        color: 'base',      fill: 'flat'    },
  ].forEach(function (cfg) {
    var $b = $('<button>').text(cfg.text);
    $('#btn-icons-row').append($b);
    $b.kendoButton({ icon: cfg.icon, themeColor: cfg.color, fillMode: cfg.fill });
  });

  // Solo ícono (sin texto)
  [
    { icon: 'search',   color: 'primary',   fill: 'solid'   },
    { icon: 'filter',   color: 'base',      fill: 'solid'   },
    { icon: 'gear',     color: 'base',      fill: 'flat'    },
    { icon: 'bell',     color: 'base',      fill: 'flat'    },
    { icon: 'plus',     color: 'success',   fill: 'solid'   },
    { icon: 'trash',    color: 'error',     fill: 'outline' },
    { icon: 'star',     color: 'warning',   fill: 'flat'    },
  ].forEach(function (cfg) {
    var $b = $('<button>');
    $('#btn-icons-only-row').append($b);
    $b.kendoButton({ icon: cfg.icon, themeColor: cfg.color, fillMode: cfg.fill });
  });

  // 10. Disabled — todos los fill modes
  allFillModes.forEach(function (fm) {
    var $b = $('<button>').text(cap(fm) + ' (disabled)');
    $('#btn-disabled-row').append($b);
    $b.kendoButton({ fillMode: fm, themeColor: 'primary', enable: false });
  });

  // 11. SplitButton — botón + flecha que abre lista de acciones secundarias
  [
    {
      id: 'split-save', text: 'Guardar', icon: 'save', color: 'primary',
      items: [
        { id: 'save-draft',  text: 'Guardar borrador',   icon: 'file'     },
        { id: 'save-export', text: 'Guardar y exportar', icon: 'download' },
        { id: 'save-copy',   text: 'Guardar copia',      icon: 'copy'     },
      ],
    },
    {
      id: 'split-pub', text: 'Publicar', icon: 'check', color: 'success',
      items: [
        { id: 'pub-now',      text: 'Publicar ahora',        icon: 'check'    },
        { id: 'pub-schedule', text: 'Programar publicación',  icon: 'calendar' },
        { id: 'pub-draft',    text: 'Guardar como borrador',  icon: 'file'     },
      ],
    },
    {
      id: 'split-actions', text: 'Acciones', icon: 'gear', color: 'base',
      items: [
        { id: 'act-edit',   text: 'Editar',   icon: 'pencil' },
        { id: 'act-delete', text: 'Eliminar', icon: 'trash'  },
      ],
    },
  ].forEach(function (cfg) {
    var $b = $('<button>', { id: cfg.id });
    $('#btn-split-row').append($b);
    $b.kendoSplitButton({ text: cfg.text, icon: cfg.icon, themeColor: cfg.color, items: cfg.items });
  });

  // 12. DropDownButton — abre un menú desplegable
  [
    {
      id: 'dd-export', text: 'Exportar', icon: 'download', color: 'primary',
      items: [
        { id: 'ex-csv',  text: 'CSV',       icon: 'file-txt'   },
        { id: 'ex-xlsx', text: 'Excel',     icon: 'file-excel' },
        { id: 'ex-pdf',  text: 'PDF',       icon: 'file-pdf'   },
        { id: 'ex-json', text: 'JSON',      icon: 'file-code'  },
      ],
    },
    {
      id: 'dd-view', text: 'Vista', icon: 'eye', color: 'base',
      items: [
        { id: 'v-grid',  text: 'Grilla',   icon: 'grid'          },
        { id: 'v-list',  text: 'Lista',    icon: 'list-unordered'},
        { id: 'v-card',  text: 'Tarjetas', icon: 'layout-1-by-4' },
      ],
    },
    {
      id: 'dd-report', text: 'Reportes', icon: 'chart-line', color: 'secondary',
      items: [
        { id: 'r-daily',   text: 'Diario'   },
        { id: 'r-weekly',  text: 'Semanal'  },
        { id: 'r-monthly', text: 'Mensual'  },
        { id: 'r-custom',  text: 'Personalizado', icon: 'gear' },
      ],
    },
  ].forEach(function (cfg) {
    var $b = $('<button>', { id: cfg.id });
    $('#btn-dropdown-row').append($b);
    $b.kendoDropDownButton({ text: cfg.text, icon: cfg.icon, themeColor: cfg.color, items: cfg.items });
  });

  // 13. ButtonGroup — selección togglable (radio o checkbox)
  var $bgWrap1 = $('<div>');
  $('#btn-group-row').append($('<div>').append(
    $('<div class="demo-label">').text('Alineación de texto (selección exclusiva)'),
    $bgWrap1
  ));
  $bgWrap1.kendoButtonGroup({
    items: [
      { text: 'Izquierda', icon: 'align-left',    togglable: true, selected: true },
      { text: 'Centro',    icon: 'align-center',  togglable: true },
      { text: 'Derecha',   icon: 'align-right',   togglable: true },
      { text: 'Justif.',   icon: 'align-justify', togglable: true },
    ],
  });

  var $bgWrap2 = $('<div>');
  $('#btn-group-row').append($('<div>').append(
    $('<div class="demo-label">').text('Vista de datos (selección exclusiva)'),
    $bgWrap2
  ));
  $bgWrap2.kendoButtonGroup({
    items: [
      { text: 'Grilla',    icon: 'grid',           togglable: true, selected: true },
      { text: 'Lista',     icon: 'list-unordered', togglable: true },
      { text: 'Tarjetas',  icon: 'layout-1-by-4',  togglable: true },
    ],
  });

  var $bgWrap3 = $('<div>');
  $('#btn-group-row').append($('<div>').append(
    $('<div class="demo-label">').text('Formato de texto (selección múltiple)'),
    $bgWrap3
  ));
  $bgWrap3.kendoButtonGroup({
    selection: 'multiple',
    items: [
      { text: 'B', icon: 'bold',      togglable: true },
      { text: 'I', icon: 'italic',    togglable: true },
      { text: 'U', icon: 'underline', togglable: true },
    ],
  });
